...
export default async function IncomingListPage() {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="مکاتبات ورودی شرکت"
        action={
          <Link href="/correspondence/incoming/new" className="btn-seal">
            <Plus className="h-4 w-4" /> ثبت نامه وارده
          </Link>
        }
      />
      ...
      {letters.length === 0 ? (
        <EmptyState
          title="هنوز نامه وارده‌ای ثبت نشده است."
          hint="برای ثبت اولین نامه صادره، روی «ثبت نامه وارده» بزنید."
          action={
            <Link href="/correspondence/incoming/new" className="btn-primary">
              <Plus className="h-4 w-4" /> ثبت نامه وارده
            </Link>
          }
        />
          ...
          <table className="w-full">
            <thead>
              <tr className="table-head">
                ...
                <th className="px-4 py-3">فرستنده</th>
              ...
              {letters.map((l) => (
                <tr key={l.id} className="table-row">
                  ...
                  <td className="px-4 py-3 text-ink-muted">
                    {(l.sender_company_id && nameOf.get(l.sender_company_id)) || l.recipient_name || "—"}
                  </td>
