// =============================================================================
// NIL Office — concurrency test for atomic project numbering (Phase 1).
//
// Simulates two authorized users finalizing two DRAFT projects at the same
// instant. Correct system => two DISTINCT PRJ- numbers.
...
//      and project_role IN ('APPROVE','ADMIN') (or app role ADMIN) — only
//      approve-tier users may call finalize_project.
//   2) export the env vars below, then:
//      node supabase/tests/concurrent-finalize-project.mjs
// =============================================================================
...
async function makeDraft(client, title) {
  const {
    data: { user },
  } = await client.auth.getUser();
  const { data, error } = await client
    .from('projects')
    .insert({ title, project_manager_id: user.id })
    .select('id')
    .single();
...
async function finalize(client, id) {
  const { data, error } = await client.rpc('finalize_project', { p_id: id });
...
const run = async () => {
  ...
  const [d1, d2] = await Promise.all([
    makeDraft(c1, 'تست هم‌زمانی پروژه ۱'),
    makeDraft(c2, 'تست هم‌زمانی پروژه ۲'),
  ]);
