      ...
      try {
        ...
        if (action.requiresConfirmation) {
          ...
          resultBlocks.push({
            ...
            toolUseId: tu.id,
            content:
              "پیشنهاد ساخته شد و منتظر تأیید کاربر است. جزئیات کامل این پیشنهاد به‌طور خودکار زیر پاسخ تو نمایش داده می‌شود — آن را در پاسخ خودت تکرار یا بازنویسی نکن. فقط اگر نکتهٔ مهمی (مثل تناقض، ابهام، یا هشداری که کاربر باید قبل از تأیید ببیند) هست، خیلی کوتاه (حداکثر دو یا سه خط) اضافه کن و از کاربر برای تأیید بپرس؛ در غیر این صورت فقط یک جملهٔ کوتاه مثل «لطفاً پیش‌نمایش زیر را بررسی و تأیید کنید» کافی است.",
          });
        } else {
